...
This repository contains the publish workflow, static seed data selection
and documentation of the first-generation C4PO ocean data pipeline,
publishing at https://oceansensing.org/ocean-data-repo/.
...
**It covers the original work in this repository**: the workflow and the
documentation.
...
**The scientific data is not ours and never was.** The static files in
`map/` and everything this pipeline publishes are fetched or derived from
the bodies that produced them and remain theirs, under their own terms:
GEBCO (bathymetry and isobaths), Natural Earth (coastline, boundaries,
glaciers and ice shelves), Marine Regions/VLIZ (EEZ boundaries), NOAA/NHC
(storm forecasts and wind probabilities), NOAA PMEL (uncrewed surface
vehicles), US IOOS, NOC/BODC, OTN and VOTO (gliders), Ifremer (Argo
floats), NOAA/NCEI and NOAA/PSL (OISST), the US Navy via HYCOM (ESPC-D-V02
